...
export class GalleryService {
  ...
  private resource: Resource<RecordSchema> = this.api.build('gallery');
  private resourceSections: Resource<RecordSchema> = this.api.build('gallery/sections');
  ...
  indexSections(params = new HttpParams()): Observable<ApiError | Records> {
    return this.currentLanguage$.pipe(
      switchMap(lang => {
        if (!isUndefined(lang) && lang.code) {
          params = params.set('lang', lang.code);
        ...
        return this.processResponse(this.resourceSections.index(params));
      })
    );
  }
