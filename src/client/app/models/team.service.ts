...
export class TeamService {
  ...
  private resource: Resource<RecordSchema> = this.api.build('team');
  ...
  index(params = new HttpParams()): Observable<ApiError | Records> {
    return this.currentLanguage$.pipe(
      switchMap(lang => {
        ...
        params = params.set('properties', 'position,FIRSTNAME,position_type,SECTION,WORK_DIRECTION');
        ...
        if (raw instanceof ApiError) {
          this.notifications
            .toast({
              ...
              timeOut: 4000,
              title: 'Ошибка API!',
              text: 'При запросе certs произошла ошибка',
